...
#  module Client
    ...
    class Sender
      ...
      def send_command(nid, cmd, value = nil, receiver = :oneline_receiver)
        #return value is tcp socket connection
        ...
        #send is Ruby's default instance method
        # this command call 'receiver' method.
        # con is argument
        # return value is 'receiver' method's return value
        ...
        # we have to use "gets" to get data
        ret
      end
      ...
      private :oneline_receiver

      def multiplelines_receiver(con)
        ret = []
        while (line = con.gets) != "END\r\n"
          return false if line =~ /ERROR/
          ret << line.chomp 
        end
        ret
      end
      private :multiplelines_receiver
